...
public class LeanAudio : object
{
    ...
    */
            ...
            if (options.vibrato != null)
            {
                for (int j = 0; j < options.vibrato.Length; j++)
                {
                    float peakMulti = Mathf.Abs(
                        Mathf.Sin(1.5708f + (passed * (1f / options.vibrato[j][0]) * Mathf.PI))
                    );
        ...
        for (int i = 0; i < audioArr.Length; i++)
        {
            ...
            if (options.modulation != null)
            {
                for (int k = 0; k < options.modulation.Length; k++)
                {
                    float peakMulti = Mathf.Abs(
                        Mathf.Sin(
                            1.5708f + (passedTime * (1f / options.modulation[k][0]) * Mathf.PI)
                        )
                    );
    ...
    {
        GameObject tempGO = new(); // create the temp object
    ...
    public static void printOutAudioClip(
        AudioClip audioClip,
